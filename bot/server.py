# bot/server.py   v2.3.1  (2025‑04‑19)
# ==========================================================
...
# ---------- Event Handler ----------
@handler.add(MessageEvent)          # ← 修正：直接指定事件 class
def on_message(event: MessageEvent):
    ...
    # /update_fut -------------------------------------------
        ...
        try:
            ...
            logging.exception(e)
            msg = f"更新失敗：{e}"
        line_api.reply_message(event.reply_token, TextMessage(text=msg))

    # /reset_fut --------------------------------------------
    elif text == "/reset_fut":
        if uid not in ADMIN_USER_IDS:
            line_api.reply_message(event.reply_token,
                                   TextMessage(text="❌ 你沒有權限執行 /reset_fut"))
            return
        COL_FUT.drop()
        try:
